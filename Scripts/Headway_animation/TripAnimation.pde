...
void loadData(){
  tripTable = loadTable("MTA_segment.csv", "header");
  ...
  for (int i=0; i<tripTable.getRowCount(); i++){ //******** take this back up to the full dataset *********
    ...
    trip[i] = new Trips(duration, startFrame, endFrame, startStation, endStation, startX, startY, endX, endY, tripid);
  }
}
...
class Trips{
  ...
  // Class constructor
  Trips(int duration, int start_frame, int end_frame, String startStation, String endStation, float startX, float startY, float endX, float endY, int tripid){
